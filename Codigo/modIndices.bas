...
'On Error GoTo Fallo
    ...
    For NPC = 1 To NumNPCs
        ...
        'If LenB(NpcData(NPC).name) <> 0 Then frmNPCs.lListadonpc.AddItem NpcData(NPC).name & " - #" & NPC
    Next
    'MsgBox "  "
...
'On Error GoTo Fallo
    ...
    Dim NPC As Integer
    Dim Leer As New clsIniReader
    frmNPCs.lListadonpc(1).Clear
    frmNPCs.lListadonpc(2).Clear
    Call Leer.Initialize(DirDats & "\NPCs.dat")
    NumNPCs = Val(Leer.GetValue("INIT", "NumNPCs"))
    'Call Leer.Initialize(DirDats & "\NPCs-HOSTILES.dat")
    'NumNPCsHOST = Val(Leer.GetValue("INIT", "NumNPCs"))
    ...
    For NPC = 1 To NumNPCs
        ...
        NpcData(NPC).GiveGLD = Val(Leer.GetValue("NPC" & NPC, "GiveGLD"))
        NpcData(NPC).GiveEXP = Val(Leer.GetValue("NPC" & NPC, "GiveEXP"))
        NpcData(NPC).NpcType = Val(Leer.GetValue("NPC" & NPC, "NpcType"))

        'If LenB(NpcData(NPC).name) <> 0 Then frmNPCs.lListadonpc.AddItem NpcData(NPC).name & " - #" & NPC
        If LenB(NpcData(NPC).name) <> 0 Then frmNPCs.lListadonpc(2).AddItem NpcData(NPC).name & " - #" & NPC & " - Tipo -" & NpcData(NPC).NpcType & " - " & " Hostil = " & NpcData(NPC).Hostile & " - " & "Oro =" & NpcData(NPC).GiveGLD & " - " & "EXP =" & NpcData(NPC).GiveEXP
    Next
    'MsgBox "  "
    'Trabajando = "Dats\NPCs-HOSTILES.dat"
    'Call Leer.Initialize(DirDats & "\NPCs-HOSTILES.dat")
    ...
    '    NpcData(NPC + 499).Head = Val(Leer.GetValue("NPC" & (NPC + 499), "Head"))
    '    NpcData(NPC + 499).Heading = Val(Leer.GetValue("NPC" & (NPC + 499), "Heading"))
    '    If LenB(NpcData(NPC + 499).name) <> 0 Then frmNPCs.lListadonpc(2).AddItem NpcData(NPC + 499).name & " - #" & (NPC + 499)
    'Next NPC
    Exit Sub
Fallo:
    MsgBox "Error al intentar cargar el NPC " & NPC & " de " & Trabajando & " en " & DirDats & vbCrLf & "Err: " & Err.Number & " - " & Err.Description, vbCritical + vbOKOnly
